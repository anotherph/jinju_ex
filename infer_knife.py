...
Discription: To infer the landmarks of one knives.
...
3) call the function to predict the center of knife
    ...
    output : the end of knife corresponding to the original image
4) call the function to predict the landmarks
    input: the predicted the end of knife, original image
...
class CenterCrop(object):
    ...
    def __call__(self, sample):
        ...
        '''bbox = [x1,y1,x2,y2]'''        
        # space=200
        bbox_0=int(position[0])-self.space
        bbox_1=int(position[1])-self.space
        bbox_2=int(position[0])+self.space
        bbox_3=int(position[1])+self.space
        
        if bbox_0<0:
            bbox_0=0
        elif bbox_1<0:
            bbox_1=0
        elif bbox_2>image.shape[1]:
            bbox_2=image.shape[1]
        elif bbox_3>image.shape[0]:
            bbox_3=image.shape[0]
        else:
            pass
        
        bbox = [int(bbox_0),int(bbox_1),int(bbox_2),int(bbox_3)]
...
def landmark_Recursion(cropped_,ori_,pose, landmarks):
...
def infer_func(sample):
    ...
    landmarks_left=infer_landmarks(2, sample_landmark, num_class=1)
    ...
    center_position_right=infer_center(1, sample_center, num_class=1)
    ...
    landmarks_right=infer_landmarks(2, sample_landmark, num_class=1)
...
def infer_center(net, sample, num_class):
    ...
    landmark_re=landmark_Recursion(300,400,sample['center'],display_result)
...
def infer_landmarks(net, sample, num_class):
    ...
    temp=predictions.detach().numpy()
    display_result=(temp[0,:].reshape(-1,2)+0.5)
    landmark_re=landmark_Recursion(300,200,sample['center'],display_result)
...
if __name__ == "__main__":       
    ...
    # path_image='/home/jekim/workspace/jinju_ex/data/0720_SGU/original_video/zed_camera/geommu4/images'
    # path_annos_tot="/home/jekim/workspace/jinju_ex/data/0720_SGU/original_video/zed_camera/geommu4/openpose"
    
    path_image='/home/jekim/workspace/jinju_ex/data/0810_KETI/dataset/images'
    path_annos_tot="/home/jekim/workspace/jinju_ex/data/0810_KETI/dataset/openpose"
    
    list_=sorted(os.listdir(path_image))
    
    ''' load pretrained network'''
    with torch.no_grad():
        net1='/home/jekim/workspace/jinju_ex/log/20220808_111340_df2op/df2op_30.pth'
    ...
    with torch.no_grad():
        net2='/home/jekim/workspace/jinju_ex/log/20220808_132120_df2op/df2op_50.pth'
        best_network2 = Network(1)
        best_network2.cuda()
    ...
    # for file_name_temp in range(13000,16000,10):
        file_name='000207'
        # load image
        # file_name='{0:05d}'.format(file_name_temp)
        name_img = os.path.join(path_image,file_name+'.jpg')     
        image = io.imread(name_img)
        
        # load pose_keypoints
        name_annos = os.path.join(path_annos_tot,file_name+'_keypoints.json')
        with open(name_annos) as f: annos = json.load(f)
               
        landmarks_pose=np.array(annos['people'][0]['pose_keypoints_2d']).reshape(-1,3)
        
        if landmarks_pose[4,:].tolist()==[0,0,0] or landmarks_pose[7,:].tolist()==[0,0,0]:     
            ...
            # plt.imshow(image)
            # plt.scatter(landmarks_pose[(4,7),0],landmarks_pose[(4,7),1])
            # plt.show()
            ...
            # plt.figure(figsize=(12,12*1920/1080))
            ax = plt.gca()
            ax.add_patch(
                  patches.Rectangle(
                    (center[0,0]-100, center[0,1]-100),
                    200,
                    200,
                    edgecolor = 'yellow',
                    # facecolor = 'red',
            ...
            # plt.scatter(landmarks[:,0], landmarks[:,1], c = 'k', s = 50)
            plt.scatter(center[:,0], center[:,1], c = 'g', s = 50)
            plt.scatter(landmarks[0,0],landmarks[0,1],c='r',s=50)
            plt.scatter(landmarks[1,0],landmarks[1,1],c='b',s=50)
            
            plt.yticks(fontsize=16)
            plt.xticks(fontsize=16)
            plt.imshow(image)
            # plt.show()
            # plt.savefig(os.path.join('/home/jekim/workspace/jinju_ex/data/0720_SGU/original_video/zed_camera/geommu4/result',file_name+'.png'))
            plt.savefig(os.path.join('/home/jekim/workspace/jinju_ex/data/0810_KETI/dataset/result',file_name+'.png'))
            
            annos={}
            annos['landmarks']=landmarks.tolist()
            annos['pose']=landmarks_pose[(4,7),:2].tolist()
                
            # with open(os.path.join('/home/jekim/workspace/jinju_ex/data/0720_SGU/original_video/zed_camera/geommu4/result',file_name+'_result.json'), "w") as json_file:
