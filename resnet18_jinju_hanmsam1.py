...
1. crop the image along to the bounding box  
2. learning the cropped images and landmarks  
...
class Jinju_Dataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        landmarks_hansam=np.array(annos['points_left']).reshape(-1,3)
        landmarks=landmarks_hansam
        ...
        for i, landmark in enumerate(sample['landmarks']) :
            if landmark[0]<0 and landmark[1]<0:
                landmarks_vis[i]=np.array([0,0])
...
class HandCrop(object):
    " crop the area of the hand in image using bbox or the position of hand "
...
class Network(nn.Module):
    ...
    #     predic = predictions.reshape(batch_size*num_class*3,-1)
        
    #     gt_1=torch.cat((landmarks_val[:,:2],landmarks_vis[:,0].unsqueeze(1)),dim=1)
    #     gt_2=torch.cat((landmarks_val[:,2:],landmarks_vis[:,2].unsqueeze(1)),dim=1)
    #     gt = torch.cat((gt_1,gt_2),dim=1)
    #     gt = gt.reshape(batch_size*num_class*3,-1)
                 
    #     loss = torch.pow((predic - gt), 2).mean()
...
if __name__ == "__main__":       
    ...
    train_img_dir = "/home/jekim/workspace/jinju_ex/data/GX010166/images"
    train_json_path ="/home/jekim/workspace/jinju_ex/data/GX010166/annos"
    ...
    dataset_train, dataset_valid = torch.utils.data.random_split(dataset, [300, 40])
    ...
    for epoch in range(1,num_epochs+1):
        ...
        with torch.no_grad():
            ...
            for step in range(1,len(valid_loader)+1):
                ...
                display_landmarks=(temp[0,:].reshape(-1,2)+0.5)
                display_landmarks=np.delete(display_landmarks,(1,3),axis=0)
                temp=predictions.cpu().detach().numpy()
                display_result=(temp[0,:].reshape(-1,2)+0.5)
