#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 22 16:43:29 2022

@author: jekim

data annotation tool 
1. read json-file (openpose)
2. read json-file (keypoint of knife and fringe)
4. write json-file including new points

"""
import os
import sys 
import cv2 
import json
import numpy as np
import matplotlib.pyplot as plt

 
if __name__ == "__main__":

    root_annos='/home/jekim/workspace/jinju_ex/data/white2/annos'
    root_openpose='/home/jekim/workspace/jinju_ex/data/white2/openpose'
    desti_annos='/home/jekim/workspace/jinju_ex/data/white2/annos_tot'

    
    for file_name_temp in os.listdir(root_annos):
        file_name=file_name_temp.split('.')[0][:6]
    # file_name='008860'
        name_annos = os.path.join(root_annos,file_name+'.json')
        name_openpose = os.path.join(root_openpose,file_name+'_keypoints.json')
              
        with open(name_openpose) as f: annos = json.load(f)
        annos_edit=annos['people'][0]
        
        # change the the order of keypoints to being same as that of cocodataset
        keypoints_openpose=np.array(annos['people'][0]['pose_keypoints_2d']).reshape(-1,3)
        keypoints_COCO=keypoints_openpose[[0,14,15,16,17,2,5,3,6,4,7,8,11,8,12,10,13],:]
        
        with open(name_annos) as f: annos = json.load(f)
        
        # add the keypoints of knife
        annos_edit['pose_keypoints_2d']=keypoints_COCO.reshape(-1).tolist()
        annos_edit['knife']=annos['knife'] # left front-end/left back-end /right front-end/right back-end
        annos_edit['fringe']=annos['fringe']
         
        cv2.destroyAllWindows()
            
        with open(os.path.join(desti_annos,file_name+'_keypoints.json'), "w") as json_file:
            json.dump(annos_edit, json_file)
