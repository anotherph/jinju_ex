...
class Jinju_Dataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        center[0,0]=center[0,0]+random.randrange(-50,50)
        center[0,1]=center[0,1]+random.randrange(-50,50)
        landmark_vis = np.zeros((1,2)) 
        
        space=100
        bbox_0=int(center[0,0])-space
        if bbox_0<0: # small x 
            bbox_0=0
        bbox_1=int(center[0,1])-space
        if bbox_1<0:
            bbox_1=0
        bbox_2=int(center[0,0])+space
        if bbox_2>image.shape[1]: # large x 
            bbox_2=image.shape[1]
        bbox_3=int(center[0,1])+space
        if bbox_3>image.shape[0]:
            bbox_3=image.shape[0]
